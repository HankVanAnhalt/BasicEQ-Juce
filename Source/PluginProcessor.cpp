...
void BasicEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    auto peakCoeeficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(sampleRate,
                                                                                chainSettings.peakFreq,
                                                                                chainSettings.peakQuality,
                                                                                juce::Decibels::decibelsToGain(chainSettings.peakGainInDecibel));
    *leftChain.get<ChainPositions::Peak>().coefficients = *peakCoeeficients;
    *rightChain.get<ChainPositions::Peak>().coefficients = *peakCoeeficients;
}
...
void BasicEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    auto chainSettings = getChainSettings(apvts);
    auto peakCoeeficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(getSampleRate(),
                                                                                chainSettings.peakFreq,
                                                                                chainSettings.peakQuality,
                                                                                juce::Decibels::decibelsToGain(chainSettings.peakGainInDecibel));
    *leftChain.get<ChainPositions::Peak>().coefficients = *peakCoeeficients;
...
juce::AudioProcessorValueTreeState::ParameterLayout BasicEQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Freq", "Peak Freq", juce::NormalisableRange<float>(20.f, 20000.f, 1.f, .25f), 750));
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Gain", "Peak Gain", juce::NormalisableRange<float>(-24.f, 24.f, .5f, 1.f), .0f));
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Quality", "Peak Quality", juce::NormalisableRange<float>(.1f, 10.f, .05f, 1.f), 1.f));
